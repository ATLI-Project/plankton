  ...
  return (
    <section className="container-wide py-24">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {latest.map((p) => (
          <Link key={p.slug} href={`/insights/${p.slug}`} className="card no-underline group block">
            <div className="text-xs text-ink/50">
              {formatDate(p.date)} · {p.readingTime}
            </div>
            <h3 className="mt-3 font-serif text-xl text-navy group-hover:text-accent transition">
              {p.title}
            </h3>
            <p className="mt-3 text-sm text-ink/70 leading-relaxed">{p.dek}</p>
          </Link>
        ))}
      </div>
    </section>
